...
selected_price = st.sidebar.slider(
    ...
    value=(price_min, price_max), step=5.0
)
...
# Apply neighbourhood + room_type filters to moon data as well
df_moon_f = df_moon.copy()
if selected_nb:
    df_moon_f = df_moon_f[df_moon_f["neighbourhood"].isin(selected_nb)]
if selected_rt:
    df_moon_f = df_moon_f[df_moon_f["room_type"].isin(selected_rt)]
if full_moon_only:
    df_moon_f = df_moon_f[df_moon_f["is_full_moon_review"] == True]

# ─── Header ───────────────────────────────────────────────────────────────────
st.info("Auteur : LOUZZA Zehair | MBA ESG Big Data & IA | MBAESG_EVALUATION_MANAGEMENT_OPERATIONNEL_2026")
...
r3.metric("Prix/Qualité",     f"{pq:.1f} EUR/pt")
...
# ── Tab 1 : Vue par quartier ──────────────────────────────────────────────────
with tab1:
    st.subheader("Distribution par quartier")
    if len(df_f) > 0:
        nb = (
            df_f.groupby("neighbourhood")
            .agg(total=("price", "count"), avg_price=("price", "mean"), avg_rating=("review_scores_rating", "mean"))
            .reset_index()
        ...
        c1, c2 = st.columns(2)
        with c1:
            fig = px.bar(nb, x="neighbourhood", y="total", color="avg_price",
                         title="Logements par quartier", color_continuous_scale="Reds")
            fig.update_layout(xaxis_tickangle=-45)
            st.plotly_chart(fig, use_container_width=True)
        with c2:
            fig2 = px.bar(nb, x="neighbourhood", y="avg_price", color="avg_rating",
                          title="Prix moyen par quartier", color_continuous_scale="Blues")
            fig2.update_layout(xaxis_tickangle=-45)
            st.plotly_chart(fig2, use_container_width=True)

        st.subheader("Ratio Prix/Qualité par quartier")
        nb["pq_ratio"] = nb["avg_price"] / nb["avg_rating"]
        fig3 = px.bar(
            nb.sort_values("pq_ratio"), x="neighbourhood", y="pq_ratio",
            title="Prix par point de note (EUR)", color="pq_ratio",
...
with tab2:
    ...
    if len(df_f) > 0:
        c1, c2 = st.columns(2)
        with c1:
            fig = px.histogram(df_f, x="price", nbins=50,
                               title="Distribution des prix",
                               color_discrete_sequence=["#FF5A5F"])
            st.plotly_chart(fig, use_container_width=True)
        with c2:
            fig2 = px.box(df_f, x="room_type", y="price",
                          title="Prix par type de logement", color="room_type")
            st.plotly_chart(fig2, use_container_width=True)

        st.subheader("Ratio Multi-annonces par quartier")
        mlt = (
            df_f.groupby("neighbourhood")
            .apply(lambda x: (x.groupby("host_name").size() > 1).mean() * 100)
            .reset_index()
        )
        mlt.columns = ["neighbourhood", "multi_pct"]
        fig3 = px.bar(
            mlt.sort_values("multi_pct", ascending=False),
            x="neighbourhood", y="multi_pct",
            title="% Hosts multi-annonces par quartier",
            color="multi_pct", color_continuous_scale="Oranges"
        )
        fig3.update_layout(xaxis_tickangle=-45)
        st.plotly_chart(fig3, use_container_width=True)
    else:
        ...
        with c1:
            if "review_date" in df_rev_f.columns:
                ...
                fig = px.line(monthly, x="month", y="count",
                              title="Avis par mois", markers=True,
                              color_discrete_sequence=["#FF5A5F"])
                fig.update_layout(xaxis_tickangle=-45)
                st.plotly_chart(fig, use_container_width=True)
        with c2:
            if "review_scores_rating" in df_f.columns:
                ...
                fig2 = px.bar(rt_rating, x="room_type", y="review_scores_rating",
                              title="Note moyenne par type",
                              color="review_scores_rating", color_continuous_scale="Greens")
                st.plotly_chart(fig2, use_container_width=True)

        st.subheader("Top 10 logements les mieux notés")
        top10 = (
            df_f[["host_name", "neighbourhood", "room_type", "price",
...
with tab4:
    ...
    if len(df_moon_f) > 0:
        ...
        with c1:
            moon_nb = (
                df_moon_f.groupby("neighbourhood").size()
                .reset_index(name="avis")
                .sort_values("avis", ascending=False)
                .head(10)
            )
            fig = px.bar(moon_nb, x="neighbourhood", y="avis",
                         title="Avis pleine lune par quartier",
                         color="avis", color_continuous_scale="Purples")
            fig.update_layout(xaxis_tickangle=-45)
            st.plotly_chart(fig, use_container_width=True)
        with c2:
            moon_rt = df_moon_f.groupby("room_type").size().reset_index(name="avis")
            fig2 = px.pie(moon_rt, values="avis", names="room_type",
                          title="Avis pleine lune par type")
            st.plotly_chart(fig2, use_container_width=True)
        ...
        fig3 = px.bar(cmp, x="label", y="count",
                      title="Volume d'avis : pleine lune vs normal",
                      color="label")
        st.plotly_chart(fig3, use_container_width=True)

        st.subheader("Derniers avis pleine lune")
        cols = [c for c in ["listing_name", "neighbourhood", "review_date",
                             "reviewer_name", "review_text"] if c in df_moon_f.columns]
        st.dataframe(df_moon_f[cols].head(20), use_container_width=True)
    else:
        st.info("Aucun avis de pleine lune trouvé.")
